...
E320MagField::E320MagField(const Acts::ActsScalar fieldS)
    : m_fieldS(fieldS) {};
...
const Acts::ActsScalar E320MagField::decayFunction(
        const Acts::ActsScalar x, const Acts::Vector4 params) const {
    return 1 / ( 
            ( 1 + exp( (params[0] - x) / params[2] ) ) * 
...
const Acts::Vector3 E320MagField::getDipole(
    ...
    const std::tuple<
        ...
        Acts::Vector4>& dipoleParams) const {
            ...
            Acts::ActsScalar a1 = 
                (pos[0] > xParams[0] && pos[0] < xParams[1]) ?
                    Acts::ActsScalar(1.0) : Acts::ActsScalar(0.0);
    
            Acts::ActsScalar a2 = decayFunction(pos[1], yParams);
    
            Acts::ActsScalar a3 = 
                (pos[0] > -1000 && pos[0] < 1000) ?
                    decayFunction(pos[1], zParams) : Acts::ActsScalar(0.0);
...
Acts::Result<Acts::Vector3> E320MagField::getField(
    ...
    MagneticFieldProvider::Cache& cache) const {
        ...
        if (gOpt.quad1Translation.z() - gOpt.quad1Bounds[2] < z &&
            ...
            gOpt.quad2Translation.z() - gOpt.quad2Bounds[2] < z &&
            ...
            gOpt.quad3Translation.z() - gOpt.quad3Bounds[2] < z &&
            ...
            gOpt.dipoleTranslation.z() - gOpt.dipoleBounds[2] < z &&
