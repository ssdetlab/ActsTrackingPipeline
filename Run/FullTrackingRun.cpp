...
int main() {
  ...
  for (auto& v : detector->volumes()) {
    ...
    for (auto& s : v->surfaces()) {
      ...
      // In global -- left X
      Acts::Vector3 leftXGlob(s->center(gctx).x() - gOpt.chipSizeX / 2,
                              s->center(gctx).y(), s->center(gctx).z());
      std::cout << "leftXGlob == " << leftXGlob.transpose() << "\n";
      Acts::Vector2 leftXLoc =
          s->globalToLocal(gctx, leftXGlob, Acts::Vector3(0, 1, 0)).value();
      std::cout << "leftXLoc == " << leftXLoc.transpose() << "\n";

      // In global -- right X
      Acts::Vector3 rightXGlob(s->center(gctx).x() + gOpt.chipSizeX / 2,
                               s->center(gctx).y(), s->center(gctx).z());
      std::cout << "rightXGlob == " << rightXGlob.transpose() << "\n";
      Acts::Vector2 rightXLoc =
          s->globalToLocal(gctx, rightXGlob, Acts::Vector3(0, 1, 0)).value();
      std::cout << "rightXLoc == " << rightXLoc.transpose() << "\n";

      Acts::Vector3 leftYGlob(s->center(gctx).x(), s->center(gctx).y(),
                              s->center(gctx).z() - gOpt.chipSizeY / 2);
      std::cout << "leftYGlob == " << leftYGlob.transpose() << "\n";
      Acts::Vector2 leftYLoc =
          s->globalToLocal(gctx, leftYGlob, Acts::Vector3(0, 1, 0)).value();
      std::cout << "leftYLoc == " << leftYLoc.transpose() << "\n";

      // In global -- right X
      Acts::Vector3 rightYGlob(s->center(gctx).x(), s->center(gctx).y(),
                               s->center(gctx).z() + gOpt.chipSizeY / 2);
      std::cout << "rightYGlob == " << rightYGlob.transpose() << "\n";
      Acts::Vector2 rightYLoc =
          s->globalToLocal(gctx, rightYGlob, Acts::Vector3(0, 1, 0)).value();
      std::cout << "rightYLoc == " << rightYLoc.transpose() << "\n";
    }
  }
