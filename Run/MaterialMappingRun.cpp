...
int main() {
  ...
  std::string gdmlPath =
      ...
      "prototype/ett_geometry_f566a577.gdml";
  ...
  for (auto& vol : detector->rootVolumes()) {
    ...
    for (auto& surf : vol->surfaces()) {
      ...
      std::cout << surf->isOnSurface(gctx, surf->center(gctx) + Acts::Vector3(0, 0, 14.0), Acts::Vector3(0, 1, 0)) << "\n";
    }
  }
...
//       "prototype/Uniform_DirectZ_TrackerOnly_256x128_1M/"
