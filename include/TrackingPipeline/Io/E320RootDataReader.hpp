...
inline Acts::Vector2 convertToLoc(const Acts::Vector3& glob,
...
class E320RootSimDataReader : public RootSimDataReader {
 ...
 private:
  ...
  inline void prepareMeasurements(const AlgorithmContext& context,
    ...
    for (int idx = 0; idx < trueHits->size(); idx++) {
      ...
      cluster.truthHits.push_back(
          SimHit(Acts::SourceLink(truthSsl), truthPars, ipParameters,
                 trackId->at(idx), parentTrackId->at(idx), runId->at(idx)));
