...
class PhoenixWriter : public IWriter {
 public:
  ...
  /// Writer name() method
  std::string name() const override { return "PhoenixWriter"; }
  ...
  ProcessCode write(const AlgorithmContext& ctx) override {
    ...
    jsonTracks["E320Event"]["Tracks"]["TrackerTracks"] =
        nlohmann::json::array();
    ...
    for (const auto& cluster : inputClusters) {
      ...
      Acts::Vector3 globalPos =
          m_cfg.surfaceAccessor(Acts::SourceLink(ssl))
              ->localToGlobal(ctx.geoContext, ssl.parametersLoc(),
                              Acts::Vector3{0, 1, 0});

      jsonHit["type"] = "Box";
      jsonHit["pos"] = {globalPos.x(), -globalPos.z(), globalPos.y(), 1, 1, 1};
    ...
    for (std::size_t tid = 0; tid < inputTracks.tracks.size(); tid++) {
      ...
      for (const auto& state : track.trackStatesReversed()) {
        ...
        Acts::Vector2 recoLocalPos = recoPars.template head<2>();
        ...
        jsonTrack["pos"].push_back(
            {recoGlobalPos.x(), -recoGlobalPos.z(), recoGlobalPos.y()});
        ...
        if (state.hasUncalibratedSourceLink()) {
          ...
          Acts::Vector2 hitLocalPos = ssl.parametersLoc();
          ...
          jsonHit["type"] = "Box";
          jsonHit["pos"] = {
              hitGlobalPos.x(), -hitGlobalPos.z(), hitGlobalPos.y(), 1, 1, 1};
 ...
 private:
  ...
  ReadDataHandle<Tracks> m_inputTracks{this, "InputTracks"};
