...
JsonTrackLookupReader::JsonTrackLookupReader(const Config& config)
    : m_cfg(config) {};

TrackLookup JsonTrackLookupReader::readLookup(const std::string& path) const {
  // Read the json file
  ...
  for (const auto& jGrid : jLookup) {
    ...
    TrackLookupGrid grid =
        Acts::GridJsonConverter::fromJson<TrackLookupAxisGen, TrackLookupPair>(
            jGrid["grid"], axisGen);
