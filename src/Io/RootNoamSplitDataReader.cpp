...
E320Io::RootNoamSplitDataReader::RootNoamSplitDataReader(
  ...
  m_eventMap.emplace_back(m_detEvent->trg_n, 0, 0);
  // Go through all entries and store the position of the events
  for (std::size_t i = m_cfg.skip + 1; i < nEntries; ++i) {
    m_chain->GetEntry(i);
    if (m_detEvent->trg_n != std::get<0>(m_eventMap.back())) {
      std::get<2>(m_eventMap.back()) = i;
      m_eventMap.emplace_back(m_detEvent->trg_n, i, i);
    }
  }
...
E320Io::RootNoamSplitDataReader::availableEvents() const {
  return {std::get<0>(m_eventMap.front()), std::get<0>(m_eventMap.back()) + 1};
}

ProcessCode E320Io::RootNoamSplitDataReader::read(
  ...
  for (auto entry = std::get<1>(*it); entry < std::get<2>(*it); entry++) {
    ...
    for (const auto& staveEv : m_detEvent->st_ev_buffer) {
      for (const auto& chipEv : staveEv.ch_ev_buffer) {
        ...
        for (const auto& pix : chipEv.hits) {
