...
RootSimClusterWriter::RootSimClusterWriter(
...
ProcessCode RootSimClusterWriter::write(const AlgorithmContext &ctx) {
    ...
    for (const auto& cluster : inputClusters) {
        ...
        const auto* surf = 
            m_cfg.surfaceAccessor(Acts::SourceLink(clusterSsl));
        
        Acts::Vector3 geoCenter = surf->localToGlobal(
            ctx.geoContext, 
            clusterSsl.parameters(), 
            Acts::Vector3(0, 1, 0));
        ...
        for (const auto& hit : cluster.truthHits) {
            const auto& hitSsl = 
                hit.sourceLink.get<SimpleSourceLink>();

            Acts::Vector3 trackHit = 
                surf->localToGlobal(
                    ctx.geoContext, 
                    hitSsl.parameters(), 
                    Acts::Vector3(0, 1, 0));
            ...
            trackHits.push_back(
                TVector3(trackHit.x(), trackHit.y(), trackHit.z()));
            
            trackId.push_back(hit.trackId);
            parentTrackId.push_back(hit.parentTrackId);
            runId.push_back(hit.runId);

            Acts::FreeVector onSurfParameters =
                Acts::transformBoundToFreeParameters(
                    *surf, 
                    ctx.geoContext, 
                    hit.truthParameters);

            Acts::ActsScalar onSurfE = 
                std::abs(onSurfParameters[Acts::eFreeQOverP]);

            Acts::Vector3 onSurfMomentum = onSurfE * 
                Acts::Vector3(
                    onSurfParameters[Acts::eFreeDir0],
