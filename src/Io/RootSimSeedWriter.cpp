...
RootSimSeedWriter::RootSimSeedWriter(
...
ProcessCode RootSimSeedWriter::write(const AlgorithmContext &ctx) {
    ...
    auto trackIds = inputTruthClusters 
    ...
    for (const auto& seed : inputSeeds) {
        const auto& sourceLinks = seed.sourceLinks 
            | std::views::transform([](const auto& sl) {
        ...
        auto signalTrackIds = pivotCluster.truthHits 
            | std::views::filter(
                [](const auto& hit) {
        ...
        auto seedTrackIds = seedClusters
            | std::views::filter([](const auto& cl) {
                    return cl.isSignal;
                })
            | std::views::transform([](const auto& cl) {
