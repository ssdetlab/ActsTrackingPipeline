...
SourceLinkGridConstructor::SourceLinkGridConstructor(
    const Config& config) : m_cfg(config) {};

std::unordered_map<Acts::GeometryIdentifier, SourceLinkGridConstructor::GridType> 
SourceLinkGridConstructor::constructGrid(
    ...
    std::vector<Acts::SourceLink> sourceLinks) {
        ...
        for (const auto& [id, surface] : m_cfg.layers) {
            ...
            AxisType 
                xAxis(-halfX, halfX, m_cfg.bins.first);
            AxisType 
                yAxis(-halfY, halfY, m_cfg.bins.second);

            GridType 
                grid(std::make_tuple(xAxis, yAxis));
        ...
        for (auto& sl : sourceLinks) {
            ...
            Acts::Vector2 localPos = 
                m_cfg.layers.at(layerId)->globalToLocal(
                    gctx, 
                    globalPos, 
                    Acts::Vector3(0, 1, 0), 
                    1e-1).value();
