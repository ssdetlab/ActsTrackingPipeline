...
std::optional<TrackLookupGrid::index_t> 
TrackLookupProvider::findClosestFilled(
    const TrackLookupGrid& grid,
    TrackLookupGrid::index_t bin, 
    const Acts::Vector2& dir) const {
        ...
        for (const auto& n : neighbours) {
            Acts::Vector2 newDir{
                static_cast<std::int32_t>(n.at(0)) - 
...
TrackLookupProvider::lookup(
    ...
    const Acts::SourceLink& pivot) const {
        ...
        auto bin = 
            m_lookup->at(geoId).localBinsFromPosition(localPos);

        if (m_lookup->at(geoId).atLocalBins(bin).first == nullptr) {
            try {
                auto res = findClosestFilled(
                    m_lookup->at(geoId), 
                    bin, {0, 0});
                bin = res.value();
            }
